...
if [[ -z "$AGENT_TOKEN" ]]; then
  # Без токена и без agent.env — значит, агента здесь никогда не ставили.
  # Чаще всего это команда обновления, запущенная на сервере панели вместо
  # VPN-сервера, поэтому говорим об этом прямо.
  echo "Агент на этом сервере не установлен (нет ${ENV_FILE}), поэтому нужен токен." >&2
  echo >&2
  echo "Если вы хотели обновить агента — запускайте эту команду на VPN-сервере," >&2
  echo "а не на сервере панели." >&2
  echo "Если это первая установка — возьмите готовую команду с токеном в панели:" >&2
  echo "  «Серверы» → у нужного сервера кнопка «Установка»." >&2
  exit 1
fi
if [[ -z "$PANEL_URL" ]]; then
