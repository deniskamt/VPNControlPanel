...
Набор шаблонов подобран под поведение российских сетей образца 2026 года.
Коротко, что про него известно:

  * длинное TCP-соединение к зарубежному серверу перестаёт передавать данные
    после ~15–20 КБ: оно не сбрасывается, а зависает, и клиент видит таймаут.
    Простой VLESS поверх TLS/WS в это и упирается;
  * XHTTP разносит трафик по нескольким обычным HTTP-запросам и в этот порог
    не упирается — это и есть главный шаблон на сегодня;
  * REALITY заимствует TLS-рукопожатие настоящего сайта, поэтому отдельный
    сертификат и домен не нужны;
...
PRESETS: List[Preset] = [
    ...
    Preset(
        ...
        description=(
            "Для сетей, где прямое соединение с зарубежным сервером не "
            "устанавливается: трафик идёт через Cloudflare. Сервер слушает "
            "без шифрования, TLS терминирует CDN — снаружи это обычный "
            "HTTPS к сайту за Cloudflare."
        ),
        protocol=ProxyType.vless,
        network=NetworkType.xhttp,
    ...
    Preset(
        ...
        notes=[
            ...
            "Сертификат самоподписанный, клиент проверяет его по отпечатку; "
            "если у ноды есть домен, добавьте в параметрах acmeDomain и "
            "acmeEmail — тогда сертификат будет настоящий",
        ],
    ),
    Preset(
    ...
    Preset(
        ...
        legacy=True,
        legacy_reason=(
            "длинное TLS-соединение к зарубежному серверу зависает после ~16 КБ"
        ),
...
# minClientVer мы больше не выставляем — и это важно.
#
# В исходниках REALITY проверка выглядит так:
#     config.MinClientVer == nil || Value(ClientVer) >= Value(MinClientVer)
# То есть без этого поля проверки нет вовсе, а с ним появляется планка.
# Клиент присылает версию своего ядра; те, что построены не на Xray, шлют
# нули — и сервер молча отправляет их на маскировочный сайт. Со стороны это
# выглядит как «протокол не работает»: рукопожатие проходит, а трафик нет.
#
# Плата за отказ от поля — ноль: значения по умолчанию у него не было.
MIN_CLIENT_VERSION = ""
...
def build_settings(
    ...
    if preset.network in (NetworkType.ws, NetworkType.httpupgrade, NetworkType.xhttp):
        ...
        if preset.network == NetworkType.xhttp:
            ...
            # «auto» выбирает stream-one — один длинный поток, который
            # зависает после ~16 КБ ровно так же, как обычный VLESS. Ради
...
def legacy_warning(inbound: Any) -> str:
    ...
    if inbound.security == SecurityType.tls:
        return (
            "длинное TLS-соединение к зарубежному серверу зависает примерно "
