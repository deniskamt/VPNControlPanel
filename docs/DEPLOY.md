...
`/opt/vpn-panel` его не меняет. Обновите его тем же скриптом, что и ставили,
только без токена — он возьмётся из `/opt/vpn-agent/agent.env`, Xray
переустанавливаться не будет:
...
Готовая команда с вашим адресом есть в панели: «Серверы» → «Установка».
Если панель в этот момент недоступна и скачать нечего, скрипт так и скажет и
оставит работающего агента на месте.
