...
«Подключения» → выбрать шаблон **VLESS + REALITY** → «Создать подключение».
Ключи, shortId и маскировку панель сгенерирует сама, JSON писать не нужно.
Из полей имеет смысл трогать только маскировочный домен — под него
подключение будет притворяться.
