...
Скрипт лежит рядом с панелью. Скопируйте его на передний сервер —
**с сервера панели**:

```bash
scp /opt/vpn-panel/scripts/setup_front.sh root@<передний сервер>:/root/
```

И запустите **на переднем сервере**, от root:

```bash
bash /root/setup_front.sh --node 78.17.173.142:443 --sni www.lovelive-anime.jp
```

`--node` — адрес ноды и порт подключения, `--sni` — маскировочный домен
(значение `sni=` из ссылки подписки). Скрипт поставит модуль `stream`,
освободит порт 443, перенесёт панель и сайты на `127.0.0.1:8443`, добавит
правило и проверит, доходит ли поток до ноды. Если nginx конфиг не примет,
скрипт откатит всё целиком — панель не пострадает.

Запускать можно сколько угодно раз, по разу на каждую ноду. Ещё есть
`--status` (показать правила), `--remove` (убрать правило) и `--udp <порт>`
для Hysteria2.

Дальше в панели: «Подписка» → «Настроить» у нужной строки → **Адрес** =
адрес переднего сервера. Больше ничего менять не нужно.

### Что этот скрипт делает руками
