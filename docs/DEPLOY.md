...
**minClientVer.** В параметры REALITY панель добавляет `minClientVer: 1.8.0`.
Без него свежее ядро пускает только клиентов не старше себя: сервер 26.7.28
отказывает клиенту 25.6.8, и со стороны это выглядит как «протокол не
работает». Плата — старые клиенты чуть заметнее для активного зондирования;
для сервиса с живыми пользователями совместимость важнее.
