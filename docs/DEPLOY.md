...
Отдельно про усиленную маскировку: её параметры едут в ссылке полем `extra`.
Клиент, который его не понимает, покажет подключение, но не соединится — для
таких есть JSON-подписка.
...
Параметры доезжают до клиента обычной ссылкой — в поле `extra`, куда клиент
на Xray-core кладёт их прямо в `xhttpSettings`; так же их передаёт 3x-ui.
Отдельных query-параметров у этих полей нет, поэтому без `extra` клиент
соединился бы с настройками по умолчанию, то есть никак.

Нужен клиент на свежем Xray-core: v2rayTun, Happ, v2rayNG, Streisand,
NekoBox. Если клиент старый и `extra` не понимает, у пользователя есть вторая
ссылка — **JSON-подписка** (`?format=json`), где настройки описаны конфигом
целиком.
